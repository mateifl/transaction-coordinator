...
public class DefaultCoordinationService implements CoordinationService {
    ...
    public MicroserviceTransaction createTransactionStep(TransactionMessage transactionMessage) {
        ...
        if(transactionMessage.getIsSuccessful())
        	...
        	microserviceTransaction.setState(DistributedTransactionStatus.ROLLEDBACK);
        ...
        if(microserviceTransaction.getIsLast())
        {
        	...
        	boolean readyToCommit = microserviceTransactionRepository.findAllByTransactionTransactionId( transactionMessage.getTransactionId() )
        		.stream()
        		.allMatch( v -> v.getState() == DistributedTransactionStatus.READY_TO_COMMIT );
        	
        	if(readyToCommit) {
        		log.info("distributed transaction {} ready to commit", transactionMessage.getTransactionId());
        		distributedTransactionOptional.get().setStatus(DistributedTransactionStatus.READY_TO_COMMIT);
